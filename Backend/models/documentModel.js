...
const { documents, firCases, users } = require('../db/schema');
const { eq, and, desc } = require('drizzle-orm');

const documentModel = {
  create: async (docData) => {
    const { fir_id, document_type, file_name, file_url, uploaded_by } = docData;
    const [doc] = await db.insert(documents)
      .values({ fir_id, document_type, file_name, file_url, uploaded_by })
      .returning();
    return doc;
  },
